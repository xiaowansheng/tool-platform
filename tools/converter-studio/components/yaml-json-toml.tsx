...
export default function YamlJsonTomlConverterTab({ inputText, onChangeInputText }: ComponentProps) {
  ...
  function convert() {
    ...
    try {
      setOutput(serializeByFormat(parseByFormat(inputText, source), target));
      setError("");
      setCopied(false);
    } catch (convertError) {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: "1.5rem" }}>
      ...
      <p className="tool-note" style={{ fontSize: "0.8rem", color: "var(--text-tertiary)" }}>
        当前实现覆盖常见扁平配置、嵌套路径和一级 section，不尝试完整替代 YAML/TOML 标准解析器。所有解析和转换操作均在本地安全运行。
      </p>
