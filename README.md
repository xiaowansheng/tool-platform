...
```bash
pnpm create-tool json-diff --name "JSON Diff" --category developer --runtime simple
```
...
```bash
pnpm create-tool my-tool --name "My Tool" --category developer --runtime simple
```
...
```text
developer, ai, text, image, video, file, network, ops, design, productivity
```
